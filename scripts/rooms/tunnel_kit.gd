...
static func build_horizontal(
	...
	parent.add_child(_box(Vector3(inner_w, WALL_T, length), Vector3(0, -WALL_T * 0.5, cz), floor_mat))
	parent.add_child(_box(Vector3(inner_w + WALL_T * 2.0, WALL_T, length + WALL_T * 2.0), Vector3(0, inner_h + WALL_T * 0.5, cz), ceil_mat, false))
	parent.add_child(_box(Vector3(WALL_T, inner_h, length), Vector3(-inner_w / 2.0 - WALL_T / 2.0, inner_h / 2.0, cz), wall_mat))
	parent.add_child(_box(Vector3(WALL_T, inner_h, length), Vector3(inner_w / 2.0 + WALL_T / 2.0, inner_h / 2.0, cz), wall_mat))
	if not open_near:
		parent.add_child(_box(Vector3(inner_w, inner_h, WALL_T), Vector3(0, inner_h / 2.0, start_z - WALL_T / 2.0), wall_mat))
	if not open_far:
		parent.add_child(_box(Vector3(inner_w, inner_h, WALL_T), Vector3(0, inner_h / 2.0, start_z + length + WALL_T * 0.5), wall_mat))
...
static func build_hub_connector(
	...
	parent.add_child(_box(Vector3(inner_w, WALL_T, depth), Vector3(0, -WALL_T * 0.5, cz), floor_mat))
	parent.add_child(_box(Vector3(inner_w + WALL_T * 2.0, WALL_T, depth + WALL_T), Vector3(0, inner_h + WALL_T * 0.5, cz), ceil_mat, false))
	parent.add_child(_box(Vector3(WALL_T, inner_h, depth), Vector3(-inner_w / 2.0 - WALL_T / 2.0, inner_h / 2.0, cz), wall_mat))
	parent.add_child(_box(Vector3(WALL_T, inner_h, depth), Vector3(inner_w / 2.0 + WALL_T / 2.0, inner_h / 2.0, cz), wall_mat))
