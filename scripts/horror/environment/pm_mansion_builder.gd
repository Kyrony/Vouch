...
static func _build_basement(root: Node3D, mats, child_markers: Array) -> void:
	...
	_GB.call("add_wall_panel", bs, Vector3(0, 1.7, 0), Vector3(0.22, 3.4, 10), mats.concrete, true, 1.5)
	_KIT.call("add_crate", bs, Vector3(-3.6, 0, -3.2), mats)
	_KIT.call("add_crate", bs, Vector3(-3.0, 0, -3.4), mats)
	_KIT.call("add_grate", bs, Vector3(2.2, 0.12, -2.4), mats)
	var pipes := Node3D.new()
	pipes.name = "Pipes"
	bs.add_child(pipes)
	_KIT.call("add_pipe_run", pipes, Vector3(-5.2, 2.6, -4.2), Vector3(5.2, 2.6, -4.2), 0.07, mats)
	_KIT.call("add_pipe_run", pipes, Vector3(-5.0, 2.4, 3.8), Vector3(-5.0, 0.4, 3.8), 0.08, mats)
	_KIT.call("add_fluorescent", bs, Vector3(0, 3.05, 0), mats)
	_add_child_marker(bs, Vector3(3.4, 0.5, 2.2), "basement", child_markers)
	root.add_child(_GEOM.call("box", Vector3(2.2, 4.1, 2.2), Vector3(3.6, -2.05, 3.2), mats.concrete))
...
static func _build_bunker(root: Node3D, mats, child_markers: Array) -> void:
	...
	_GB.call("add_wall_panel", bk, Vector3(-2.4, 1.9, 0), Vector3(0.22, 3.8, 10), mats.concrete, true, 2.0)

	var work := Node3D.new()
	work.name = "Workbench"
	bk.add_child(work)
	_KIT.call("add_workbench", work, Vector3(3.8, 0, 3.4), mats)

	var shelves := Node3D.new()
	shelves.name = "Shelves"
	bk.add_child(shelves)
	_KIT.call("add_shelf", shelves, Vector3(4.6, 0, -2.8), mats)
	_KIT.call("add_shelf", shelves, Vector3(4.6, 0, -1.2), mats)
	_KIT.call("add_crate", bk, Vector3(3.2, 0, -3.6), mats)
	_KIT.call("add_crate", bk, Vector3(3.7, 0, -3.2), mats)
	_KIT.call("add_grate", bk, Vector3(0.8, 0.12, 1.2), mats)

	var pipes := Node3D.new()
	pipes.name = "Pipes"
	bk.add_child(pipes)
	_KIT.call("add_pipe_run", pipes, Vector3(-5.4, 3.1, -4.4), Vector3(5.4, 3.1, -4.4), 0.08, mats)
	_KIT.call("add_pipe_run", pipes, Vector3(5.2, 3.1, -4.4), Vector3(5.2, 3.1, 4.2), 0.07, mats)
	_KIT.call("add_pipe_run", pipes, Vector3(-5.3, 2.8, 2.4), Vector3(-5.3, 0.5, 2.4), 0.09, mats)
	_KIT.call("add_pipe_run", pipes, Vector3(-1.6, 3.0, -4.2), Vector3(-1.6, 3.0, 3.6), 0.06, mats)
	# Elbow clusters
	_KIT.call("add_cyl", pipes, 0.1, 0.35, Vector3(5.2, 3.1, -4.4), mats.metal)
	_KIT.call("add_box", pipes, Vector3(0.55, 0.7, 0.22), Vector3(-5.2, 1.4, 3.6), mats.metal)

	var neon := Node3D.new()
	neon.name = "NeonStrips"
	bk.add_child(neon)
	_KIT.call("add_neon_strip", neon, Vector3(-2.2, 2.4, 0), Vector3(0.06, 0.06, 7.4), mats.neon_red)
	_KIT.call("add_neon_strip", neon, Vector3(2.6, 2.6, -4.6), Vector3(6.2, 0.06, 0.06), mats.fluorescent)
	_KIT.call("add_neon_strip", neon, Vector3(0.2, 0.18, 4.6), Vector3(4.8, 0.05, 0.05), mats.neon_red)

	var lights := Node3D.new()
	lights.name = "Fluorescent"
	bk.add_child(lights)
	_KIT.call("add_fluorescent", lights, Vector3(1.6, 3.45, 1.2), mats)
	_KIT.call("add_fluorescent", lights, Vector3(-3.4, 3.45, -1.6), mats)

	var panel := Node3D.new()
	panel.name = "ElectricalPanel"
	bk.add_child(panel)
	_KIT.call("add_box", panel, Vector3(0.55, 0.8, 0.12), Vector3(2.2, 1.4, 4.7), mats.metal)
	_KIT.call("add_neon_strip", panel, Vector3(2.2, 1.55, 4.76), Vector3(0.12, 0.08, 0.04), mats.neon_red)
...
static func _build_utility_closet(bk: Node3D, mats, child_markers: Array) -> void:
	...
	_KIT.call("add_shelf", closet_shelves, Vector3(-1.15, 0, -1.15), mats)
	_KIT.call("add_shelf", closet_shelves, Vector3(0.15, 0, -1.15), mats)
	_KIT.call("add_crate", util, Vector3(1.0, 0, -1.0), mats)

	var warm := OmniLight3D.new()
	warm.name = "ClosetLamp"
	warm.position = Vector3(0, 2.1, 0)
	warm.light_color = Color(1.0, 0.82, 0.42)
	warm.light_energy = 0.7
	warm.omni_range = 4.2
	util.add_child(warm)
	_KIT.call("add_neon_strip", util, Vector3(0, 2.25, -1.45), Vector3(1.6, 0.05, 0.05), mats.fluorescent)
