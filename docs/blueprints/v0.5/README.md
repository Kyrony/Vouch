...
| HUD icon pack (soft-go) | wired in `assets/horror/hud/` | Kyle-locked meters: horror-vial **health over yellow stamina** empty tracks (eng fill %). Top-right signal + battery widgets. Right **5-slot item rail** (octagon wells, blue select bloom). No fear bar / chips / bottom hotbar. **Phone LED** (not a torch) / E interact. |
